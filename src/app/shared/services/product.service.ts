...
//		this.favouriteProducts.remove(key);
  ...
  */
	...
	getLocalCartProducts(): Product[] {
		...
		console.log ('constProducts ' + localStorage.getItem(myshoppincar))
		const products: Product[] = JSON.parse(localStorage.getItem(myshoppincar)) || [];

		return products;
	}catch{
		console.log('Error entry product')
	}
	...
	increaseProduct(product: Product) {
		...
		if (product.productId == product["productId"]) {
				this.quantity = product.productQuatity;
				this.quantity = this.quantity + 1;
				product.productQuatity = this.quantity;
			}


		console.log('increase product test: ' + this.quantity)		

	}catch{
		console.log('Error increase product')	
	}
	}
	...
	decreaseProduct(product: Product) {
		try{
			...
			console.log('decrease product test: ' + this.quantity)		
